		...
		methods: {
			...
			async sendComment() {
				...
				var senddata = {
					...
					comment_type: that.relaydata.comment_type || 0,
				...
				// 不主动获取评论
				// if (that.relaydata.comment_cj > 1) {
				// 	that.getComment(that.relaydata.reply_comment_id);
				// } else {
				// 	that.getComment();
				// }
			},
			...
			async getComment(comment_id) {
				...
				if (res_comment.length > 0) {
					...
					if (!comment_id) {
						...
						var likepl = await db.collection("opendb-news-likeplTaolun").where({
							article_id: that.zydata._id,
							user_id: uid
						}).get();
