<template>
	<view class="o-dropdown-item" v-if="active" @touchmove.stop.prevent="() => {}" @tap.stop.prevent="() => {}">
		<block v-if="!$slots.default && !$slots.$default">
			<scroll-view scroll-y="true" :style="{
				height: $u.addUnit(height)
			}">
				<view class="o-dropdown-item__options">
					<u-cell-group>
...
<script>
	...
	export default {
		...
		computed: {
			// 监听props是否发生了变化，有些值需要传递给父组件o-dropdown，无法双向绑定
			propsChange() {
		...
		methods: {
			init() {
				// 获取父组件o-dropdown
				let parent = this.$u.$parent.call(this, 'o-dropdown');
				if (parent) {
					this.parent = parent;
