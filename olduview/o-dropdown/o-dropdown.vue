<template>
	<view class="o-dropdown">
		<view class="o-dropdown__menu" :style="{
			...
			<view class="o-dropdown__menu__item" v-for="(item, index) in menuList" :key="index" @tap.stop="menuClick(index)">
				<view class="u-flex" style="display: flex;">
					...
					<view class="o-dropdown__menu__item__arrow" :class="{
						'o-dropdown__menu__item__arrow--rotate': index === current
					}">
		...
		<view class="o-dropdown__content" :style="[contentStyle, {
...
<script>
	...
	export default {
		...
		created() {
			// 引用所有子组件(o-dropdown-item)的this，不能在data中声明变量，否则在微信小程序会造成循环引用而报错
			this.children = [];
		},
		mounted() {
		...
		methods: {
			...
			getContentHeight() {
				...
				this.$uGetRect('.o-dropdown__menu').then(res => {
