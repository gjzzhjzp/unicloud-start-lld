<template>
	<view class="o-section">
		<view class="o-section__title" :style="{
			fontWeight: bold ? 'bold' : 'normal',
			color: color,
			fontSize: fontSize + 'rpx',
			paddingLeft: showLine ? (fontSize * 0.7) + 'rpx' : 0
		}" :class="{
			'o-section--line': showLine
		}">
			<view class="o-section__title__icon-wrap u-flex" :style="[lineStyle]" v-if="showLine">
				<u-icon top="0" name="column-line" :size="fontSize * 1.25" bold :color="lineColor ? lineColor : color"></u-icon>
			</view>
			<text class="u-flex o-section__title__text">{{title}}</text>
		</view>
		<view class="o-section__right-info" v-if="right || $slots.right" :style="{
			color: subColor
		}" @tap="rightClick"> 
			<slot name="right" v-if="$slots.right" />
			<block v-else>
				{{subTitle}}
				<view class="o-section__right-info__icon-arrow u-flex" v-if="arrow">
...
<script>
	 ...
	 * @example <o-section title="今日热门" :right="false"></o-section>
	 */
	export default {
		name: "o-section",
		props: {
