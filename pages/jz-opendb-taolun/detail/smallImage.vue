<template>
	<view class="smallImage">
		<view v-if="showNum>=index" class="smallImage1" @click="previewOpen(data)">
			...
			<u-image :src="data.url" loading-icon="/static/center/zheng.png" error-icon="/static/center/error_zheng.png" :border-radius="8" @click="previewOpen(data)" @load="loadSuccess(index)" @error="errorImg(index)"
				mode="aspectFill"></u-image>
		</view>
		<view v-else class="smallImage1">
			<image src="/static/center/zheng.png"  :style="{'border-radius':'4px'}" mode="aspectFill"></image>
		</view>
