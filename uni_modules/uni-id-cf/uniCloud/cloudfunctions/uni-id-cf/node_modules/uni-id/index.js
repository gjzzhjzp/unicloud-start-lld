...
async function Kr({
	...
	switch (o.length) {
		case 0:
			return {
				code: 10004, message: "用户缺少唯一标识（包括但不限于登录名、手机号、邮箱）"
			};
		case 1:
...
var dn = Object.freeze({
	...
	login: async function({
		...
		r && r.length || (r = ["username"]), r.length > 1 && console.warn(
			"检测到当前使用queryField匹配多字段进行登录操作，需要注意：uni-id并未限制登录名不能是手机号或邮箱，需要开发者自行限制。否则可能出现用户输入abc@xx.com会同时匹配到邮箱为此值的用户和登录名为此值的用户，导致登录失败"
			);
		const {
