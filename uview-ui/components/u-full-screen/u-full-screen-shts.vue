<template>
	<u-modal v-model="show" confirm-text="同意" cancel-text="退出" :show-cancel-button="true" title="提示" @cancel="cancel"
		@confirm="confirm">
	...
	export default {
		data() {
			return {
				...
				content: `
					为加强注册登录验证，现开启发微博审核。
					用户需在指定地方填入【微博主页链接地址+验证资料】且在微博发送一条【指定的微博】（仅审核时验证使用），待管理组审核通过方可登录进入。
